module intrinsic-extrinsic_solution where
...
-- The Plan
-- 1) Questions from lecture
-- 2) Questions for last tutorial
-- 3) Repeat intrinsically typed syntax 
-- 4) Prove some substitution lemmas
-- 5) If we have time: intriniscally scoped 
...
module Intrinsic where
  ...
  -- t de bruijn index: t proof that type `A` occurs in context `Γ`.
  ...
  exts σ Z     = ` Z
  exts σ (S x) = rename wk (σ x)
    where wk : ∀{Γ t} → Rename Γ (Γ , t)
          wk x = S x

  -- …and lift to terms (simultaneous substitution).
  subst : ∀ {Γ Δ} → Subst Γ Δ → ∀ {t} → Γ ⊢ t → Δ ⊢ t
  subst σ (` x)          = σ x
  subst σ (ƛ e)          = ƛ (subst (exts σ) e)
  subst σ (e₁ · e₂)      = (subst σ e₁) · (subst σ e₂)
  subst σ `zero          = `zero
  subst σ (`suc e)       = `suc subst σ e
  subst σ (case e e₁ e₂) = case (subst σ e) (subst σ e₁) (subst (exts σ) e₂)
  subst σ (μ e)          = μ subst (exts σ) e
  ...
  _[_] : ∀ {Γ t t′} →
    ...
    Γ ⊢ t →
    Γ ⊢ t′
  _[_] {Γ} {t} e e′ = subst (single e′) e
    where single :  ∀ {Γ t} → Γ ⊢ t →  Subst (Γ , t) Γ  
          single e′ Z     = e′
          single e′ (S x) = ` x
  ...
  open import Data.Product as P using (∃-syntax) 
  progress : ∀ {t} (e : ∅ ⊢ t) →
    Value e ⊎ ∃[ e′ ] (e ↪ e′)
  progress (ƛ e)          = inj₁ V-ƛ
  progress (e₁ · e₂)      with progress e₁ | progress e₂ 
  ... | (inj₁ V-ƛ) | inj₁ v                = inj₂ (P._,_ _ (β-ƛ v)) 
  ... | (inj₁ V-ƛ) | inj₂ (P._,_ _ e₂↪e₂′) = inj₂ (P._,_ _ (ξ-·₂ V-ƛ e₂↪e₂′))
  ... | inj₂ (P._,_ _ e₁↪e₁′) | _          = inj₂ (P._,_ _ (ξ-·₁ e₁↪e₁′)) 
  progress (`zero)        = inj₁ V-zero
  progress (`suc e)       with progress e 
  ... | (inj₁ v)              = inj₁ (V-suc v)
  ... | (inj₂ (P._,_ _ e↪e′)) = inj₂ (`suc _ P., ξ-suc e↪e′)
  progress (case e e₁ e₂) with progress e 
  ... | inj₁ V-zero           = inj₂ (e₁ P., β-zero)
  ... | inj₁ (V-suc v)        = inj₂ ((e₂ [ _ ]) P., β-suc v)
  ... | (inj₂ (P._,_ _ e↪e′)) = inj₂ (case _ e₁ e₂ P., ξ-case e↪e′)
  progress (μ e) = inj₂ ((e [ μ e ]) P., β-μ)
